...
function renderAddExerciseCategories(): void {
  ...
  container.innerHTML = mainCategories.map(main => {
    ...
    const targetBadge = isTargetCategory
      ? '<span class="ml-2 text-xs bg-warm-primary text-white px-2 py-0.5 rounded-full">Target</span>'
      : '';
      ...
      <div class="mb-4">
        <button onclick="app.toggleAddExerciseCategory('${main.name}')" class="flex justify-between items-center w-full py-2 text-left">
          ...
          <div class="flex items-center">
            ...
            <span id="add-${main.name}-arrow" class="text-warm-text-secondary">${isExpanded ? '&#9660;' : '&#9654;'}</span>
          </div>
        </button>
        <div id="add-${main.name}-exercises" class="space-y-2 mt-2 ${isExpanded ? '' : 'hidden'}">
          ${exercises.map(e => {
            ...
            const buttonClass = inWorkout
              ? 'w-full bg-warm-bg rounded-2xl p-3 text-left opacity-50 cursor-not-allowed'
              : 'w-full bg-warm-card rounded-2xl p-3 text-left hover:bg-warm-bg shadow-warm-sm transition-colors';
            const inWorkoutBadge = inWorkout
              ? '<span class="text-xs bg-warm-border text-warm-text-secondary px-2 py-0.5 rounded-full ml-2">In workout</span>'
              : '';
            return `
              <button ${inWorkout ? 'disabled' : `onclick="app.addExerciseToWorkout('${e.name.replace(/'/g, "\\'")}')"`} class="${buttonClass}" data-exercise-in-workout="${inWorkout}">
                ...
                ${prText ? `<div class="text-xs text-warm-secondary mt-1">${prText}</div>` : ''}
              </button>
            `;
          }).join('')}
        </div>
...
export function setWorkoutExerciseUnit(unit: 'lbs' | 'kg'): void {
  ...
  $('workout-exercise-unit-lbs').className = unit === 'lbs' ? 'bg-warm-primary text-white px-4 py-2 rounded-xl text-sm' : 'bg-warm-border text-warm-text px-4 py-2 rounded-xl text-sm hover:bg-warm-border/80';
  $('workout-exercise-unit-kg').className = unit === 'kg' ? 'bg-warm-primary text-white px-4 py-2 rounded-xl text-sm' : 'bg-warm-border text-warm-text px-4 py-2 rounded-xl text-sm hover:bg-warm-border/80';
}
