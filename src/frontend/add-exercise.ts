...
function renderAddExerciseCategories(): void {
  ...
  container.innerHTML = mainCategories.map(main => {
    ...
    const categoryLabelClass = isTargetCategory
      ...
      : 'font-medium text-[#F1F5F9]/80';
    const targetBadge = isTargetCategory
      ? '<span class="ml-2 text-xs bg-gradient-to-r from-[#8B5CF6] to-[#06B6D4] text-white px-2 py-0.5 rounded-lg">Target</span>'
      : '';
      ...
      <div class="mb-4">
        <button onclick="app.toggleAddExerciseCategory('${main.name}')" class="flex justify-between items-center w-full py-2 text-left">
          ...
          <div class="flex items-center">
            ...
            <span id="add-${main.name}-arrow" class="text-[#64748B]">${isExpanded ? '&#9660;' : '&#9654;'}</span>
          </div>
        </button>
        <div id="add-${main.name}-exercises" class="space-y-2 mt-2 ${isExpanded ? '' : 'hidden'}">
          ${exercises.map(e => {
            ...
            const buttonClass = inWorkout
              ? 'w-full bg-white/[0.02] rounded-xl p-3 text-left opacity-50 cursor-not-allowed border border-white/5'
              : 'w-full glass-card rounded-xl p-3 text-left hover:bg-white/10 transition-all';
            const inWorkoutBadge = inWorkout
              ? '<span class="text-xs bg-white/5 text-[#64748B] px-2 py-0.5 rounded-lg ml-2">In workout</span>'
              : '';
            return `
              <button ${inWorkout ? 'disabled' : `onclick="app.addExerciseToWorkout('${e.name.replace(/'/g, "\\'")}')"`} class="${buttonClass}" data-exercise-in-workout="${inWorkout}">
                ...
                ${prText ? `<div class="text-xs text-yellow-400 pr-star-glow mt-1">${prText}</div>` : ''}
              </button>
            `;
          }).join('')}
        </div>
...
export function setWorkoutExerciseUnit(unit: 'lbs' | 'kg'): void {
  ...
  $('workout-exercise-unit-lbs').className = unit === 'lbs' ? 'neon-unit-active px-4 py-2 rounded-lg text-sm' : 'neon-unit-inactive px-4 py-2 rounded-lg text-sm';
  $('workout-exercise-unit-kg').className = unit === 'kg' ? 'neon-unit-active px-4 py-2 rounded-lg text-sm' : 'neon-unit-inactive px-4 py-2 rounded-lg text-sm';
}
