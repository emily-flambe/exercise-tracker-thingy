...
function renderAddExerciseCategories(): void {
  ...
  container.innerHTML = mainCategories.map(main => {
    ...
    const targetBadge = isTargetCategory
      ? '<span class="ml-2 text-xs bg-[#FF0000] text-white px-2 py-0.5 rounded-sm uppercase tracking-wider">Target</span>'
      : '';
      ...
      <div class="mb-4">
        <button onclick="app.toggleAddExerciseCategory('${main.name}')" class="flex justify-between items-center w-full py-2 text-left">
          ...
          <div class="flex items-center">
            ...
            <span id="add-${main.name}-arrow" class="text-[#888888]">${isExpanded ? '&#9660;' : '&#9654;'}</span>
          </div>
        </button>
        <div id="add-${main.name}-exercises" class="space-y-2 mt-2 ${isExpanded ? '' : 'hidden'}">
          ${exercises.map(e => {
            ...
            const buttonClass = inWorkout
              ? 'w-full bg-[#0A0A0A] border border-[#2A2A2A] rounded-sm p-3 text-left opacity-50 cursor-not-allowed'
              : 'w-full bg-[#141414] border border-[#2A2A2A] rounded-sm p-3 text-left hover:border-[#888888]';
            const inWorkoutBadge = inWorkout
              ? '<span class="text-xs bg-[#2A2A2A] text-[#888888] px-2 py-0.5 rounded-sm ml-2">In workout</span>'
              : '';
            return `
...
export function setWorkoutExerciseUnit(unit: 'lbs' | 'kg'): void {
  ...
  $('workout-exercise-unit-lbs').className = unit === 'lbs' ? 'bg-[#FF0000] px-4 py-2 rounded-sm text-sm font-bold' : 'bg-[#1A1A1A] border border-[#2A2A2A] px-4 py-2 rounded-sm text-sm hover:bg-[#2A2A2A]';
  $('workout-exercise-unit-kg').className = unit === 'kg' ? 'bg-[#FF0000] px-4 py-2 rounded-sm text-sm font-bold' : 'bg-[#1A1A1A] border border-[#2A2A2A] px-4 py-2 rounded-sm text-sm hover:bg-[#2A2A2A]';
}
