...
function renderAddExerciseCategories(): void {
  ...
  container.innerHTML = mainCategories.map(main => {
    ...
    const categoryLabelClass = isTargetCategory
      ? 'font-medium text-[#F43F5E]'
      : 'font-medium text-[#1F1315]';
    const targetBadge = isTargetCategory
      ? '<span class="ml-2 text-xs bg-[#F43F5E] text-white px-2 py-0.5 rounded-full">Target</span>'
      : '';
      ...
      <div class="mb-4">
        <button onclick="app.toggleAddExerciseCategory('${main.name}')" class="flex justify-between items-center w-full py-2 text-left">
          ...
          <div class="flex items-center">
            ...
            <span id="add-${main.name}-arrow" class="text-[#9C8B8B]">${isExpanded ? '&#9660;' : '&#9654;'}</span>
          </div>
        </button>
        <div id="add-${main.name}-exercises" class="space-y-2 mt-2 ${isExpanded ? '' : 'hidden'}">
          ${exercises.map(e => {
            ...
            const buttonClass = inWorkout
              ? 'w-full bg-[#FFF9F5] rounded-xl p-3 text-left opacity-50 cursor-not-allowed'
              : 'w-full bg-white rounded-xl p-3 text-left hover:bg-[#FFFCFA] shadow-[0_1px_4px_rgba(244,63,94,0.04)]';
            const inWorkoutBadge = inWorkout
              ? '<span class="text-xs bg-[#F0E4E0] text-[#9C8B8B] px-2 py-0.5 rounded-full ml-2">In workout</span>'
              : '';
            return `
              <button ${inWorkout ? 'disabled' : `onclick="app.addExerciseToWorkout('${e.name.replace(/'/g, "\\'")}')"`} class="${buttonClass}" data-exercise-in-workout="${inWorkout}">
                ...
                ${prText ? `<div class="text-xs text-[#F97316] mt-1">${prText}</div>` : ''}
              </button>
            `;
          }).join('')}
        </div>
...
export function setWorkoutExerciseUnit(unit: 'lbs' | 'kg'): void {
  ...
  $('workout-exercise-unit-lbs').className = unit === 'lbs' ? 'bg-[#F43F5E] text-white px-4 py-2 rounded-xl text-sm shadow-[0_2px_6px_rgba(244,63,94,0.2)]' : 'bg-[#FFFCFA] border border-[#F0E4E0] text-[#1F1315] px-4 py-2 rounded-xl text-sm hover:bg-[#F0E4E0]';
  $('workout-exercise-unit-kg').className = unit === 'kg' ? 'bg-[#F43F5E] text-white px-4 py-2 rounded-xl text-sm shadow-[0_2px_6px_rgba(244,63,94,0.2)]' : 'bg-[#FFFCFA] border border-[#F0E4E0] text-[#1F1315] px-4 py-2 rounded-xl text-sm hover:bg-[#F0E4E0]';
}
