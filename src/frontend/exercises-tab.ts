...
function updateExerciseTabSortButtons(): void {
  ...
  if (exerciseTabSort.field === 'alpha') {
    alphaBtn.className = 'text-terminal-green';
    alphaBtn.textContent = exerciseTabSort.asc ? 'A-Z' : 'Z-A';
    recentBtn.className = 'text-terminal-dim';
    recentBtn.textContent = 'Recent';
  } else {
    recentBtn.className = 'text-terminal-green';
    recentBtn.textContent = exerciseTabSort.asc ? 'Recent' : 'Oldest';
    alphaBtn.className = 'text-terminal-dim';
    ...
    return `
      <div class="mb-4">
        <button onclick="app.toggleCategory('${main.name}')" class="flex justify-between items-center w-full py-2 text-left">
          <span class="font-medium text-terminal-green">${main.name}</span>
          <span class="text-terminal-dim text-sm mr-2">${exercises.length}</span>
          <span id="${main.name}-arrow" class="text-terminal-dim">${isExpanded ? '&#9660;' : '&#9654;'}</span>
        </button>
        <div id="${main.name}-exercises" class="space-y-2 mt-2 ${isExpanded ? '' : 'hidden'}">
          ${exercises.map(e => {
            ...
            return `
              <button onclick="app.showEditExercise('${e.name.replace(/'/g, "\\'")}')" class="w-full bg-terminal-surface border border-terminal-border rounded-none p-3 text-left hover:border-terminal-green flex justify-between items-center">
                <span class="font-medium">${e.name}</span>
                <div class="text-right">
                  ${lastLoggedText ? `<span class="text-xs text-terminal-dim">${lastLoggedText}</span>` : ''}
                </div>
              </button>
            `;
          }).join('')}
...
export function showEditExercise(exerciseName: string): void {
  ...
  if (exercisePRs.length > 0) {
    historyHTML += `
      <div class="mb-3">
        <div class="text-terminal-amber text-xs font-medium mb-2">* PERSONAL RECORDS</div>
        <div class="flex text-terminal-dim text-xs mb-1 font-mono">
        ...
        ${exercisePRs.sort((a, b) => b.achieved_at - a.achieved_at).slice(0, 5).map(pr => `
          <div class="flex text-sm py-1 font-mono">
            <span class="w-20 text-terminal-dim">${formatDate(pr.achieved_at)}</span>
  ...
  if (recentSets.length > 0) {
    historyHTML += `
      <div>
        <div class="text-terminal-dim text-xs font-medium mb-2">RECENT SETS</div>
        <div class="flex text-terminal-dim text-xs mb-1 font-mono">
          <span class="w-20">DATE</span>
          <span class="w-16">WEIGHT</span>
          <span class="flex-1">REPS</span>
        ...
        ${recentSets.map(s => `
          <div class="flex text-sm py-1 font-mono">
            <span class="w-20 text-terminal-dim">${formatDate(s.date)}</span>
            <span class="w-16">${s.weight} ${exercise.unit}</span>
            <span class="flex-1">${s.reps}${s.note ? ` <span class="text-terminal-dim text-xs">${s.note}</span>` : ''}</span>
          </div>
        `).join('')}
      </div>
    `;
  ...
  if (historyHTML) {
    ...
    historyList.innerHTML = '<p class="text-terminal-dim text-sm">No history yet</p>';
  }
...
function renderWeightChart(data: Array<{ date: number; maxWeight: number }>, unit: string, containerId: string): void {
  ...
  const svg = `
    <svg width="${width}" height="${height}" class="w-full">
      <!-- Grid lines -->
      ${yLabels.map(v => `<line x1="${padding.left}" y1="${yScale(v)}" x2="${width - padding.right}" y2="${yScale(v)}" stroke="rgba(0, 255, 65, 0.15)" stroke-width="1"/>`).join('')}

      <!-- Y-axis labels -->
      ${yLabels.map(v => `<text x="${padding.left - 5}" y="${yScale(v) + 4}" text-anchor="end" fill="#00AA2A" font-size="10" font-family="IBM Plex Mono, monospace">${v}</text>`).join('')}

      <!-- X-axis labels -->
      <text x="${padding.left}" y="${height - 5}" text-anchor="start" fill="#00AA2A" font-size="10" font-family="IBM Plex Mono, monospace">${formatShortDate(data[0].date)}</text>
      <text x="${width - padding.right}" y="${height - 5}" text-anchor="end" fill="#00AA2A" font-size="10" font-family="IBM Plex Mono, monospace">${formatShortDate(data[data.length - 1].date)}</text>

      <!-- Line -->
      <path d="${pathPoints}" fill="none" stroke="#00FF41" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>

      <!-- Points -->
      ${data.map((d, i) => `<circle cx="${xScale(i)}" cy="${yScale(d.maxWeight)}" r="3" fill="#00FF41"/>`).join('')}
    </svg>
    <div class="text-center text-xs text-terminal-dim mt-1 font-mono">Max weight (${unit}) over time</div>
  `;
...
export function setExerciseUnit(unit: 'lbs' | 'kg'): void {
  ...
  $('exercise-unit-lbs').className = unit === 'lbs' ? 'bg-terminal-green text-black px-4 py-2 rounded-none text-sm font-bold font-mono' : 'bg-terminal-surface border border-terminal-border text-terminal-green px-4 py-2 rounded-none text-sm hover:bg-terminal-green hover:text-black font-mono';
  $('exercise-unit-kg').className = unit === 'kg' ? 'bg-terminal-green text-black px-4 py-2 rounded-none text-sm font-bold font-mono' : 'bg-terminal-surface border border-terminal-border text-terminal-green px-4 py-2 rounded-none text-sm hover:bg-terminal-green hover:text-black font-mono';
}
