...
function updateExerciseTabSortButtons(): void {
  ...
  if (exerciseTabSort.field === 'alpha') {
    alphaBtn.className = 'text-lime-400 uppercase tracking-wider';
    alphaBtn.textContent = exerciseTabSort.asc ? 'A-Z' : 'Z-A';
    recentBtn.className = 'text-gray-400 uppercase tracking-wider';
    recentBtn.textContent = 'Recent';
  } else {
    recentBtn.className = 'text-lime-400 uppercase tracking-wider';
    recentBtn.textContent = exerciseTabSort.asc ? 'Recent' : 'Oldest';
    alphaBtn.className = 'text-gray-400 uppercase tracking-wider';
...
export function renderExerciseCategories(): void {
  ...
  container.innerHTML = mainCategories.map(main => {
    ...
    return `
      <div class="mb-4">
        <button onclick="app.toggleCategory('${main.name}')" class="flex justify-between items-center w-full py-2 text-left border-b-2 border-gray-800">
          <span class="font-bold uppercase tracking-wider text-gray-300">${main.name}</span>
          <span class="text-gray-600 text-sm mr-2 font-mono">${exercises.length}</span>
          <span id="${main.name}-arrow" class="text-gray-500">${isExpanded ? '&#9660;' : '&#9654;'}</span>
        </button>
        <div id="${main.name}-exercises" class="space-y-2 mt-2 ${isExpanded ? '' : 'hidden'}">
          ${exercises.map(e => {
            ...
            return `
              <button onclick="app.showEditExercise('${e.name.replace(/'/g, "\\'")}')" class="w-full bg-surface border-2 border-gray-700 rounded-none p-3 text-left hover:border-gray-500 flex justify-between items-center">
                <span class="font-medium">${e.name}</span>
                <div class="text-right">
                  ${lastLoggedText ? `<span class="text-xs text-gray-600 font-mono">${lastLoggedText}</span>` : ''}
                </div>
              </button>
            `;
          }).join('')}
...
export function showEditExercise(exerciseName: string): void {
  ...
  if (exercisePRs.length > 0) {
    historyHTML += `
      <div class="mb-3">
        <div class="text-yellow-400 text-xs font-bold mb-2 uppercase tracking-wider">★ PERSONAL RECORDS</div>
        <div class="flex text-gray-600 text-xs mb-1 uppercase tracking-wider">
        ...
        ${exercisePRs.sort((a, b) => b.achieved_at - a.achieved_at).slice(0, 5).map(pr => `
          <div class="flex text-sm py-1 font-mono">
            <span class="w-20 text-gray-500">${formatDate(pr.achieved_at)}</span>
  ...
  if (recentSets.length > 0) {
    historyHTML += `
      <div>
        <div class="text-gray-400 text-xs font-bold mb-2 uppercase tracking-wider">RECENT SETS</div>
        <div class="flex text-gray-600 text-xs mb-1 uppercase tracking-wider">
          <span class="w-20">DATE</span>
          <span class="w-16">WEIGHT</span>
          <span class="flex-1">REPS</span>
        ...
        ${recentSets.map(s => `
          <div class="flex text-sm py-1 font-mono">
            <span class="w-20 text-gray-500">${formatDate(s.date)}</span>
            <span class="w-16">${s.weight} ${exercise.unit}</span>
            <span class="flex-1">${s.reps}${s.note ? ` <span class="text-gray-600 text-xs">${s.note}</span>` : ''}</span>
          </div>
        `).join('')}
      </div>
    `;
...
function renderWeightChart(data: Array<{ date: number; maxWeight: number }>, unit: string, containerId: string): void {
  ...
  const svg = `
    <svg width="${width}" height="${height}" class="w-full">
      <!-- Grid lines -->
      ${yLabels.map(v => `<line x1="${padding.left}" y1="${yScale(v)}" x2="${width - padding.right}" y2="${yScale(v)}" stroke="#333333" stroke-width="1"/>`).join('')}

      <!-- Y-axis labels -->
      ${yLabels.map(v => `<text x="${padding.left - 5}" y="${yScale(v) + 4}" text-anchor="end" fill="#666666" font-size="10" font-family="monospace">${v}</text>`).join('')}

      <!-- X-axis labels -->
      <text x="${padding.left}" y="${height - 5}" text-anchor="start" fill="#666666" font-size="10" font-family="monospace">${formatShortDate(data[0].date)}</text>
      <text x="${width - padding.right}" y="${height - 5}" text-anchor="end" fill="#666666" font-size="10" font-family="monospace">${formatShortDate(data[data.length - 1].date)}</text>

      <!-- Line -->
      <path d="${pathPoints}" fill="none" stroke="#BFFF00" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>

      <!-- Points -->
      ${data.map((d, i) => `<circle cx="${xScale(i)}" cy="${yScale(d.maxWeight)}" r="3" fill="#BFFF00"/>`).join('')}
    </svg>
    <div class="text-center text-xs text-gray-600 mt-1 uppercase tracking-wider font-mono">Max weight (${unit}) over time</div>
  `;
...
export function setExerciseUnit(unit: 'lbs' | 'kg'): void {
  ...
  $('exercise-unit-lbs').className = unit === 'lbs' ? 'bg-lime-400 text-black font-bold px-4 py-2 rounded-none text-sm uppercase' : 'bg-surface border-2 border-gray-600 px-4 py-2 rounded-none text-sm hover:bg-gray-800 uppercase';
  $('exercise-unit-kg').className = unit === 'kg' ? 'bg-lime-400 text-black font-bold px-4 py-2 rounded-none text-sm uppercase' : 'bg-surface border-2 border-gray-600 px-4 py-2 rounded-none text-sm hover:bg-gray-800 uppercase';
}
