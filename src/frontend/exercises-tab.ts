...
function updateExerciseTabSortButtons(): void {
  ...
  if (exerciseTabSort.field === 'alpha') {
    alphaBtn.className = 'text-sunset-rose';
    alphaBtn.textContent = exerciseTabSort.asc ? 'A-Z' : 'Z-A';
    recentBtn.className = 'text-sunset-muted';
    recentBtn.textContent = 'Recent';
  } else {
    recentBtn.className = 'text-sunset-rose';
    recentBtn.textContent = exerciseTabSort.asc ? 'Recent' : 'Oldest';
    alphaBtn.className = 'text-sunset-muted';
    ...
    return `
      <div class="mb-4">
        <button onclick="app.toggleCategory('${main.name}')" class="flex justify-between items-center w-full py-2 text-left">
          <span class="font-medium text-sunset-text">${main.name}</span>
          <span class="text-sunset-muted text-sm mr-2">${exercises.length}</span>
          <span id="${main.name}-arrow" class="text-sunset-muted">${isExpanded ? '&#9660;' : '&#9654;'}</span>
        </button>
        <div id="${main.name}-exercises" class="space-y-2 mt-2 ${isExpanded ? '' : 'hidden'}">
          ${exercises.map(e => {
            ...
            return `
              <button onclick="app.showEditExercise('${e.name.replace(/'/g, "\\'")}')" class="w-full bg-sunset-bg border border-white/10 rounded-xl p-3 text-left hover:bg-white/5 flex justify-between items-center transition-colors duration-200">
                <span class="font-medium">${e.name}</span>
                <div class="text-right">
                  ${lastLoggedText ? `<span class="text-xs text-sunset-muted">${lastLoggedText}</span>` : ''}
                </div>
              </button>
            `;
          }).join('')}
  ...
  results.innerHTML = filtered.map(e => {
    return `
      <button onclick="app.showEditExercise('${e.name.replace(/'/g, "\\'")}')" class="w-full bg-sunset-bg border border-white/10 rounded-xl p-3 text-left hover:bg-white/5 flex justify-between items-center transition-colors duration-200">
        ...
        <span class="text-sunset-muted text-xs">&#9654;</span>
      </button>
    `;
  }).join('');
}
...
export function showEditExercise(exerciseName: string): void {
  ...
  if (exercisePRs.length > 0) {
    historyHTML += `
      <div class="mb-3">
        <div class="text-orange-400 text-xs font-medium mb-2 pr-star-glow">★ PERSONAL RECORDS</div>
        <div class="flex text-sunset-muted text-xs mb-1">
        ...
        ${exercisePRs.sort((a, b) => b.achieved_at - a.achieved_at).slice(0, 5).map(pr => `
          <div class="flex text-sm py-1">
            <span class="w-20 text-sunset-muted">${formatDate(pr.achieved_at)}</span>
  ...
  if (recentSets.length > 0) {
    historyHTML += `
      <div>
        <div class="text-sunset-muted text-xs font-medium mb-2">RECENT SETS</div>
        <div class="flex text-sunset-muted text-xs mb-1">
          <span class="w-20">DATE</span>
          <span class="w-16">WEIGHT</span>
          <span class="flex-1">REPS</span>
        ...
        ${recentSets.map(s => `
          <div class="flex text-sm py-1">
            <span class="w-20 text-sunset-muted">${formatDate(s.date)}</span>
            <span class="w-16">${s.weight} ${exercise.unit}</span>
            <span class="flex-1">${s.reps}${s.note ? ` <span class="text-sunset-muted text-xs">${s.note}</span>` : ''}</span>
          </div>
        `).join('')}
      </div>
    `;
  ...
  if (historyHTML) {
    ...
    historyList.innerHTML = '<p class="text-sunset-muted text-sm">No history yet</p>';
  }
...
function renderWeightChart(data: Array<{ date: number; maxWeight: number }>, unit: string, containerId: string): void {
  ...
  const svg = `
    <svg width="${width}" height="${height}" class="w-full">
      <!-- Grid lines -->
      ${yLabels.map(v => `<line x1="${padding.left}" y1="${yScale(v)}" x2="${width - padding.right}" y2="${yScale(v)}" stroke="rgba(255,255,255,0.1)" stroke-width="1"/>`).join('')}
      ...
      <!-- Gradient definition -->
      <defs>
        <linearGradient id="sunset-line" x1="0%" y1="0%" x2="100%" y2="0%">
      ...
      <!-- Line -->
      <path d="${pathPoints}" fill="none" stroke="url(#sunset-line)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>

      <!-- Points -->
      ${data.map((d, i) => `<circle cx="${xScale(i)}" cy="${yScale(d.maxWeight)}" r="3" fill="#F43F5E"/>`).join('')}
    </svg>
    <div class="text-center text-xs text-sunset-muted mt-1">Max weight (${unit}) over time</div>
  `;
...
export function setExerciseUnit(unit: 'lbs' | 'kg'): void {
  ...
  $('exercise-unit-lbs').className = unit === 'lbs' ? 'sunset-gradient px-4 py-2 rounded-lg text-sm text-white' : 'bg-sunset-bg border border-white/10 px-4 py-2 rounded-lg text-sm text-sunset-muted hover:bg-white/5 transition-colors duration-200';
  $('exercise-unit-kg').className = unit === 'kg' ? 'sunset-gradient px-4 py-2 rounded-lg text-sm text-white' : 'bg-sunset-bg border border-white/10 px-4 py-2 rounded-lg text-sm text-sunset-muted hover:bg-white/5 transition-colors duration-200';
}
