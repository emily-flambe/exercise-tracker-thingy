    ...
    return `
      <div class="mb-4">
        <button onclick="app.toggleCategory('${main.name}')" class="flex justify-between items-center w-full py-2 text-left">
          <span class="font-bold text-black uppercase tracking-wider">${main.name}</span>
          <span class="text-gray-500 text-sm mr-2">${exercises.length}</span>
          <span id="${main.name}-arrow" class="text-black">${isExpanded ? '&#9660;' : '&#9654;'}</span>
        </button>
        <div id="${main.name}-exercises" class="space-y-2 mt-2 ${isExpanded ? '' : 'hidden'}">
          ${exercises.map(e => {
            ...
            return `
              <button onclick="app.showEditExercise('${e.name.replace(/'/g, "\\'")}')" class="w-full bg-white border-2 border-black rounded-sm p-3 text-left hover:bg-gray-50 flex justify-between items-center">
...
export function showEditExercise(exerciseName: string): void {
  ...
  if (exercisePRs.length > 0) {
    historyHTML += `
      <div class="mb-3">
        <div class="text-swiss-red text-xs font-bold mb-2 uppercase tracking-wider">★ PERSONAL RECORDS</div>
        <div class="flex text-gray-500 text-xs mb-1 font-bold uppercase tracking-wider">
        ...
        ${exercisePRs.sort((a, b) => b.achieved_at - a.achieved_at).slice(0, 5).map(pr => `
          <div class="flex text-sm py-1 font-mono border-b border-gray-200">
            <span class="w-20 text-gray-500">${formatDate(pr.achieved_at)}</span>
            <span class="w-16 font-bold">${pr.weight} ${exercise.unit}</span>
  ...
  if (recentSets.length > 0) {
    historyHTML += `
      <div>
        <div class="text-black text-xs font-bold mb-2 uppercase tracking-wider">RECENT SETS</div>
        <div class="flex text-gray-500 text-xs mb-1 font-bold uppercase tracking-wider">
          <span class="w-20">DATE</span>
          <span class="w-16">WEIGHT</span>
          <span class="flex-1">REPS</span>
  ...
  if (historyHTML) {
    ...
    historyList.innerHTML = '<p class="text-gray-500 text-sm uppercase tracking-wider">No history yet</p>';
  }
...
function renderWeightChart(data: Array<{ date: number; maxWeight: number }>, unit: string, containerId: string): void {
  ...
  const svg = `
    <svg width="${width}" height="${height}" class="w-full">
      <!-- Grid lines -->
      ${yLabels.map(v => `<line x1="${padding.left}" y1="${yScale(v)}" x2="${width - padding.right}" y2="${yScale(v)}" stroke="#E0E0E0" stroke-width="1"/>`).join('')}

      <!-- Y-axis labels -->
      ${yLabels.map(v => `<text x="${padding.left - 5}" y="${yScale(v) + 4}" text-anchor="end" fill="#666666" font-size="10">${v}</text>`).join('')}

      <!-- X-axis labels -->
      <text x="${padding.left}" y="${height - 5}" text-anchor="start" fill="#666666" font-size="10">${formatShortDate(data[0].date)}</text>
      <text x="${width - padding.right}" y="${height - 5}" text-anchor="end" fill="#666666" font-size="10">${formatShortDate(data[data.length - 1].date)}</text>

      <!-- Line -->
      <path d="${pathPoints}" fill="none" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>

      <!-- Points -->
      ${data.map((d, i) => `<circle cx="${xScale(i)}" cy="${yScale(d.maxWeight)}" r="3" fill="#FF0000"/>`).join('')}
    </svg>
    <div class="text-center text-xs text-gray-500 mt-1 uppercase tracking-wider">Max weight (${unit}) over time</div>
  `;
...
export function setExerciseUnit(unit: 'lbs' | 'kg'): void {
  ...
  $('exercise-unit-lbs').className = unit === 'lbs' ? 'bg-black text-white px-4 py-2 rounded-sm text-sm font-bold uppercase' : 'bg-white border-2 border-black px-4 py-2 rounded-sm text-sm font-bold uppercase hover:bg-gray-100';
  $('exercise-unit-kg').className = unit === 'kg' ? 'bg-black text-white px-4 py-2 rounded-sm text-sm font-bold uppercase' : 'bg-white border-2 border-black px-4 py-2 rounded-sm text-sm font-bold uppercase hover:bg-gray-100';
}
