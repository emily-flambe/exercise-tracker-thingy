    ...
    return `
      <div class="mb-4">
        <button onclick="app.toggleCategory('${main.name}')" class="flex justify-between items-center w-full py-2 text-left">
          <span class="font-medium text-[#3D2F2F]">${main.name}</span>
          <span class="text-[#8A7B72] text-sm mr-2">${exercises.length}</span>
          <span id="${main.name}-arrow" class="text-[#8A7B72]">${isExpanded ? '&#9660;' : '&#9654;'}</span>
        </button>
        <div id="${main.name}-exercises" class="space-y-2 mt-2 ${isExpanded ? '' : 'hidden'}">
          ${exercises.map(e => {
            ...
            return `
              <button onclick="app.showEditExercise('${e.name.replace(/'/g, "\\'")}')" class="w-full bg-[#F5F0EA] rounded-xl p-3 text-left hover:bg-[#EDE6DD] flex justify-between items-center border border-[#DDD5CB] transition-colors duration-200">
                <span class="font-medium">${e.name}</span>
                <div class="text-right">
                  ${lastLoggedText ? `<span class="text-xs text-[#8A7B72]">${lastLoggedText}</span>` : ''}
                </div>
              </button>
            `;
          }).join('')}
  ...
  results.innerHTML = filtered.map(e => {
    return `
      <button onclick="app.showEditExercise('${e.name.replace(/'/g, "\\'")}')" class="w-full bg-[#F5F0EA] rounded-xl p-3 text-left hover:bg-[#EDE6DD] flex justify-between items-center border border-[#DDD5CB] transition-colors duration-200">
        ...
        <span class="text-[#8A7B72] text-xs">&#9654;</span>
      </button>
    `;
  }).join('');
}
...
export function showEditExercise(exerciseName: string): void {
  ...
  if (exercisePRs.length > 0) {
    historyHTML += `
      <div class="mb-3">
        <div class="text-[#E0832A] text-xs font-medium mb-2">★ PERSONAL RECORDS</div>
        <div class="flex text-[#8A7B72] text-xs mb-1">
        ...
        ${exercisePRs.sort((a, b) => b.achieved_at - a.achieved_at).slice(0, 5).map(pr => `
          <div class="flex text-sm py-1">
            <span class="w-20 text-[#8A7B72]">${formatDate(pr.achieved_at)}</span>
  ...
  if (recentSets.length > 0) {
    historyHTML += `
      <div>
        <div class="text-[#8A7B72] text-xs font-medium mb-2">RECENT SETS</div>
        <div class="flex text-[#8A7B72] text-xs mb-1">
          <span class="w-20">DATE</span>
          <span class="w-16">WEIGHT</span>
          <span class="flex-1">REPS</span>
        ...
        ${recentSets.map(s => `
          <div class="flex text-sm py-1">
            <span class="w-20 text-[#8A7B72]">${formatDate(s.date)}</span>
            <span class="w-16">${s.weight} ${exercise.unit}</span>
            <span class="flex-1">${s.reps}${s.note ? ` <span class="text-[#8A7B72] text-xs">${s.note}</span>` : ''}</span>
          </div>
        `).join('')}
      </div>
    `;
  ...
  if (historyHTML) {
    ...
    historyList.innerHTML = '<p class="text-[#8A7B72] text-sm">No history yet</p>';
  }
...
function renderWeightChart(data: Array<{ date: number; maxWeight: number }>, unit: string, containerId: string): void {
  ...
  const svg = `
    <svg width="${width}" height="${height}" class="w-full">
      <!-- Grid lines -->
      ${yLabels.map(v => `<line x1="${padding.left}" y1="${yScale(v)}" x2="${width - padding.right}" y2="${yScale(v)}" stroke="#DDD5CB" stroke-width="1"/>`).join('')}

      <!-- Y-axis labels -->
      ${yLabels.map(v => `<text x="${padding.left - 5}" y="${yScale(v) + 4}" text-anchor="end" fill="#8A7B72" font-size="10">${v}</text>`).join('')}

      <!-- X-axis labels -->
      <text x="${padding.left}" y="${height - 5}" text-anchor="start" fill="#8A7B72" font-size="10">${formatShortDate(data[0].date)}</text>
      <text x="${width - padding.right}" y="${height - 5}" text-anchor="end" fill="#8A7B72" font-size="10">${formatShortDate(data[data.length - 1].date)}</text>

      <!-- Line -->
      <path d="${pathPoints}" fill="none" stroke="#E63B57" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>

      <!-- Points -->
      ${data.map((d, i) => `<circle cx="${xScale(i)}" cy="${yScale(d.maxWeight)}" r="3" fill="#E63B57"/>`).join('')}
    </svg>
    <div class="text-center text-xs text-[#8A7B72] mt-1">Max weight (${unit}) over time</div>
  `;
...
export function setExerciseUnit(unit: 'lbs' | 'kg'): void {
  ...
  $('exercise-unit-lbs').className = unit === 'lbs' ? 'bg-[#E63B57] text-white px-4 py-2 rounded-xl text-sm transition-colors duration-200' : 'bg-[#EDE6DD] border border-[#DDD5CB] text-[#3D2F2F] px-4 py-2 rounded-xl text-sm hover:bg-[#E6DFD6] transition-colors duration-200';
  $('exercise-unit-kg').className = unit === 'kg' ? 'bg-[#E63B57] text-white px-4 py-2 rounded-xl text-sm transition-colors duration-200' : 'bg-[#EDE6DD] border border-[#DDD5CB] text-[#3D2F2F] px-4 py-2 rounded-xl text-sm hover:bg-[#E6DFD6] transition-colors duration-200';
}
