    ...
    return `
      <div class="mb-4">
        <button onclick="app.toggleCategory('${main.name}')" class="flex justify-between items-center w-full py-2 text-left">
          <span class="font-bold text-white">${main.name}</span>
          <span class="text-[#888888] text-sm mr-2">${exercises.length}</span>
          <span id="${main.name}-arrow" class="text-[#888888]">${isExpanded ? '&#9660;' : '&#9654;'}</span>
        </button>
        <div id="${main.name}-exercises" class="space-y-2 mt-2 ${isExpanded ? '' : 'hidden'}">
          ${exercises.map(e => {
            ...
            return `
              <button onclick="app.showEditExercise('${e.name.replace(/'/g, "\\'")}')" class="w-full bg-[#141414] border border-[#2A2A2A] rounded-sm p-3 text-left hover:border-[#888888] flex justify-between items-center">
  ...
  results.innerHTML = filtered.map(e => {
    return `
      <button onclick="app.showEditExercise('${e.name.replace(/'/g, "\\'")}')" class="w-full bg-[#141414] border border-[#2A2A2A] rounded-sm p-3 text-left hover:border-[#888888] flex justify-between items-center">
        ...
        <span class="text-[#888888] text-xs">&#9654;</span>
      </button>
    `;
  }).join('');
}
...
export function showEditExercise(exerciseName: string): void {
      ...
      <div class="mb-3">
        ...
        ${exercisePRs.sort((a, b) => b.achieved_at - a.achieved_at).slice(0, 5).map(pr => `
          <div class="flex text-sm py-1 font-mono">
            <span class="w-20 text-[#888888]">${formatDate(pr.achieved_at)}</span>
  ...
  if (recentSets.length > 0) {
    historyHTML += `
      <div>
        <div class="text-[#888888] text-xs font-bold mb-2 uppercase tracking-wider">RECENT SETS</div>
        <div class="flex text-[#888888] text-xs mb-1 uppercase tracking-wider">
          <span class="w-20">DATE</span>
          <span class="w-16">WEIGHT</span>
          <span class="flex-1">REPS</span>
        ...
        ${recentSets.map(s => `
          <div class="flex text-sm py-1 font-mono">
            <span class="w-20 text-[#888888]">${formatDate(s.date)}</span>
            <span class="w-16">${s.weight} ${exercise.unit}</span>
            <span class="flex-1">${s.reps}${s.note ? ` <span class="text-[#888888] text-xs">${s.note}</span>` : ''}</span>
          </div>
        `).join('')}
      </div>
    `;
  ...
  if (historyHTML) {
    ...
    historyList.innerHTML = '<p class="text-[#888888] text-sm">No history yet</p>';
  }
...
function renderWeightChart(data: Array<{ date: number; maxWeight: number }>, unit: string, containerId: string): void {
  ...
  const svg = `
    <svg width="${width}" height="${height}" class="w-full">
      <!-- Grid lines -->
      ${yLabels.map(v => `<line x1="${padding.left}" y1="${yScale(v)}" x2="${width - padding.right}" y2="${yScale(v)}" stroke="#2A2A2A" stroke-width="1"/>`).join('')}

      <!-- Y-axis labels -->
      ${yLabels.map(v => `<text x="${padding.left - 5}" y="${yScale(v) + 4}" text-anchor="end" fill="#888888" font-size="10">${v}</text>`).join('')}

      <!-- X-axis labels -->
      <text x="${padding.left}" y="${height - 5}" text-anchor="start" fill="#888888" font-size="10">${formatShortDate(data[0].date)}</text>
      <text x="${width - padding.right}" y="${height - 5}" text-anchor="end" fill="#888888" font-size="10">${formatShortDate(data[data.length - 1].date)}</text>

      <!-- Line -->
      <path d="${pathPoints}" fill="none" stroke="#FF0000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>

      <!-- Points -->
      ${data.map((d, i) => `<circle cx="${xScale(i)}" cy="${yScale(d.maxWeight)}" r="3" fill="#FF0000"/>`).join('')}
    </svg>
    <div class="text-center text-xs text-[#888888] mt-1">Max weight (${unit}) over time</div>
  `;
...
export function setExerciseUnit(unit: 'lbs' | 'kg'): void {
  ...
  $('exercise-unit-lbs').className = unit === 'lbs' ? 'bg-[#FF0000] px-4 py-2 rounded-sm text-sm font-bold' : 'bg-[#1A1A1A] border border-[#2A2A2A] px-4 py-2 rounded-sm text-sm hover:bg-[#2A2A2A]';
  $('exercise-unit-kg').className = unit === 'kg' ? 'bg-[#FF0000] px-4 py-2 rounded-sm text-sm font-bold' : 'bg-[#1A1A1A] border border-[#2A2A2A] px-4 py-2 rounded-sm text-sm hover:bg-[#2A2A2A]';
}
