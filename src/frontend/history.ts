...
export function renderHistory(): void {
  ...
  let html = `
    <div class="mb-4 flex items-center justify-between">
      <button onclick="app.changeCalendarMonth(-1)" class="text-warm-primary hover:text-warm-primary-hover p-2">
      ...
      <div class="flex flex-col items-center">
        ...
        ${!isCurrentMonth ? '<button onclick="app.goToToday()" class="text-xs text-warm-primary hover:text-warm-primary-hover mt-1">Today</button>' : ''}
      </div>
      <button onclick="app.changeCalendarMonth(1)" class="text-warm-primary hover:text-warm-primary-hover p-2">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].forEach(day => {
    html += `<div class="text-center text-xs text-warm-text-secondary py-1">${day}</div>`;
  });
  html += '</div>';
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (hasWorkouts) {
      if (hasActiveFilter && matchesFilter) {
        cellClass += ' bg-warm-secondary text-white hover:bg-warm-secondary/80 cursor-pointer';
      } else {
        cellClass += ' bg-warm-primary text-white hover:bg-warm-primary-hover cursor-pointer';
      }
    } else {
      cellClass += ' bg-warm-card shadow-warm-sm';
    }

    if (isToday) {
      cellClass += ' ring-2 ring-warm-success';
    }
  ...
  ALL_MUSCLE_GROUPS.forEach(category => {
    ...
    const pillClass = isSelected
      ? 'bg-warm-secondary text-white'
      : 'bg-warm-card text-warm-text shadow-warm-sm hover:bg-warm-bg';
...
export function showDayWorkouts(dateStr: string): void {
  ...
  let html = `
    <div>
      <button onclick="app.renderHistory()" class="text-warm-primary hover:text-warm-primary-hover mb-4 flex items-center gap-2 transition-colors">
  ...
  workouts.forEach(w => {
    ...
    html += `
      <div class="bg-warm-card rounded-2xl p-4 cursor-pointer hover:bg-warm-bg shadow-warm-sm transition-colors" onclick="app.editWorkout('${w.id}')">
        <div class="font-medium">${time}</div>
        <div class="text-sm text-warm-text-secondary">${w.exercises.length} exercises</div>
        <div class="text-xs text-warm-text-secondary mt-1">${exerciseNames}${more}</div>
        <div class="mt-3 pt-3 border-t border-warm-border flex justify-end">
          ${isDeleting ? `
            <div class="flex items-center gap-2" onclick="event.stopPropagation()">
              <span class="text-warm-danger text-sm">Delete?</span>
              <button onclick="app.confirmDeleteWorkout('${w.id}')" class="bg-warm-danger hover:bg-warm-danger-hover text-white text-sm px-2 py-1 rounded-xl transition-colors">Yes</button>
              <button onclick="app.cancelDeleteWorkout()" class="text-warm-text-secondary text-sm hover:text-warm-text transition-colors">No</button>
            </div>
          ` : `
            <button onclick="event.stopPropagation(); app.showDeleteWorkoutConfirm('${w.id}')" class="text-warm-text-secondary text-sm hover:text-warm-danger transition-colors">Delete</button>
          `}
        </div>
      </div>
    `;
