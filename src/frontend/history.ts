...
export function renderHistory(): void {
  ...
  let html = `
    <div class="mb-4 flex items-center justify-between">
      <button onclick="app.changeCalendarMonth(-1)" class="text-[#F43F5E] hover:text-[#E11D48] p-2 transition-colors duration-200">
      ...
      <div class="flex flex-col items-center">
        ...
        ${!isCurrentMonth ? '<button onclick="app.goToToday()" class="text-xs text-[#F43F5E] hover:text-[#E11D48] mt-1 transition-colors duration-200">Today</button>' : ''}
      </div>
      <button onclick="app.changeCalendarMonth(1)" class="text-[#F43F5E] hover:text-[#E11D48] p-2 transition-colors duration-200">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].forEach(day => {
    html += `<div class="text-center text-xs text-[#9C8B8B] py-1">${day}</div>`;
  });
  html += '</div>';
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (hasWorkouts) {
      if (hasActiveFilter && matchesFilter) {
        cellClass += ' bg-[#F97316] hover:bg-orange-600 cursor-pointer text-white';
      } else {
        cellClass += ' bg-[#F43F5E] hover:bg-[#E11D48] cursor-pointer text-white';
      }
    } else {
      cellClass += ' bg-white text-[#1F1315]';
    }
  ...
  ALL_MUSCLE_GROUPS.forEach(category => {
    ...
    const pillClass = isSelected
      ? 'bg-[#F97316] text-white shadow-[0_2px_6px_rgba(249,115,22,0.25)]'
      : 'bg-white text-[#1F1315] border border-[#F0E4E0] hover:bg-[#FFFCFA]';
...
export function showDayWorkouts(dateStr: string): void {
  ...
  let html = `
    <div>
      <button onclick="app.renderHistory()" class="text-[#F43F5E] hover:text-[#E11D48] mb-4 flex items-center gap-2 transition-colors duration-200">
  ...
  workouts.forEach(w => {
    ...
    html += `
      <div class="bg-white rounded-2xl p-4 cursor-pointer hover:bg-[#FFFCFA] shadow-[0_2px_8px_rgba(244,63,94,0.06)] transition-colors duration-200" onclick="app.editWorkout('${w.id}')">
        <div class="font-medium">${time}</div>
        <div class="text-sm text-[#9C8B8B]">${w.exercises.length} exercises</div>
        <div class="text-xs text-[#9C8B8B] mt-1">${exerciseNames}${more}</div>
        <div class="mt-3 pt-3 border-t border-[#F0E4E0] flex justify-end">
          ${isDeleting ? `
            <div class="flex items-center gap-2" onclick="event.stopPropagation()">
              <span class="text-red-500 text-sm">Delete?</span>
              <button onclick="app.confirmDeleteWorkout('${w.id}')" class="bg-red-500 hover:bg-red-600 text-white text-sm px-2 py-1 rounded-xl">Yes</button>
              <button onclick="app.cancelDeleteWorkout()" class="text-[#9C8B8B] text-sm hover:text-[#1F1315]">No</button>
            </div>
          ` : `
            <button onclick="event.stopPropagation(); app.showDeleteWorkoutConfirm('${w.id}')" class="text-[#9C8B8B] text-sm hover:text-red-500 transition-colors duration-200">Delete</button>
          `}
        </div>
      </div>
    `;
