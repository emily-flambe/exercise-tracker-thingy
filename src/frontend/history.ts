...
export function renderHistory(): void {
  ...
  let html = `
    <div class="mb-4 flex items-center justify-between">
      <button onclick="app.changeCalendarMonth(-1)" class="text-sunset-rose hover:text-sunset-orange p-2 transition-colors duration-200">
      ...
      <div class="flex flex-col items-center">
        ...
        ${!isCurrentMonth ? '<button onclick="app.goToToday()" class="text-xs text-sunset-rose hover:text-sunset-orange mt-1 transition-colors duration-200">Today</button>' : ''}
      </div>
      <button onclick="app.changeCalendarMonth(1)" class="text-sunset-rose hover:text-sunset-orange p-2 transition-colors duration-200">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].forEach(day => {
    html += `<div class="text-center text-xs text-sunset-muted py-1">${day}</div>`;
  });
  html += '</div>';
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    let cellClass = 'aspect-square flex flex-col items-center justify-center rounded-xl text-sm relative transition-colors duration-200';

    if (hasWorkouts) {
      if (hasActiveFilter && matchesFilter) {
        cellClass += ' sunset-gradient cursor-pointer text-white';
      } else {
        cellClass += ' bg-sunset-rose/80 hover:bg-sunset-rose cursor-pointer text-white';
      }
    } else {
      cellClass += ' bg-sunset-bg';
    }
  ...
  ALL_MUSCLE_GROUPS.forEach(category => {
    ...
    const pillClass = isSelected
      ...
      : 'bg-sunset-bg border border-white/10 text-sunset-muted hover:bg-white/5';
    html += `<button onclick="app.toggleCalendarFilter('${category}')" class="px-3 py-1 rounded-full text-xs font-medium transition-all duration-200 ${pillClass}">${category}</button>`;
  });
  html += '</div>';
...
export function showDayWorkouts(dateStr: string): void {
  ...
  let html = `
    <div>
      <button onclick="app.renderHistory()" class="text-sunset-rose hover:text-sunset-orange mb-4 flex items-center gap-2 transition-colors duration-200">
  ...
  workouts.forEach(w => {
    ...
    html += `
      <div class="bg-sunset-card border border-white/10 rounded-xl p-4 cursor-pointer hover:bg-white/5 transition-colors duration-200" onclick="app.editWorkout('${w.id}')">
        <div class="font-medium">${time}</div>
        <div class="text-sm text-sunset-muted">${w.exercises.length} exercises</div>
        <div class="text-xs text-sunset-muted mt-1">${exerciseNames}${more}</div>
        <div class="mt-3 pt-3 border-t border-white/10 flex justify-end">
          ${isDeleting ? `
            <div class="flex items-center gap-2" onclick="event.stopPropagation()">
              <span class="text-red-400 text-sm">Delete?</span>
              <button onclick="app.confirmDeleteWorkout('${w.id}')" class="bg-red-600 hover:bg-red-700 text-white text-sm px-2 py-1 rounded-lg transition-colors duration-200">Yes</button>
              <button onclick="app.cancelDeleteWorkout()" class="text-sunset-muted text-sm hover:text-sunset-text transition-colors duration-200">No</button>
            </div>
          ` : `
            <button onclick="event.stopPropagation(); app.showDeleteWorkoutConfirm('${w.id}')" class="text-sunset-muted text-sm hover:text-red-400 transition-colors duration-200">Delete</button>
          `}
        </div>
      </div>
    `;
