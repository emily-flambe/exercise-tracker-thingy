...
function renderCategorySelectionGrid(): void {
  ...
  grid.innerHTML = ALL_MUSCLE_GROUPS.map(category => {
    ...
    const selectedClass = isSelected
      ? 'bg-[#E63B57] border-[#E63B57] text-white'
      : 'bg-[#F5F0EA] border-[#DDD5CB] text-[#3D2F2F] hover:bg-[#EDE6DD]';
    return `
      <button onclick="app.toggleTargetCategory('${category}')" class="p-3 rounded-xl border-2 text-sm font-medium transition-colors duration-200 ${selectedClass}">
        ${category}
      </button>
    `;
  }).join('');
  ...
  list.innerHTML = exercises.map((ex, i) => {
    ...
    let setsHtml = `
      <div class="text-sm">
        ${ex.sets.length > 0 ? `
          ${ex.sets.map((set, si) => {
            ...
            const setCheckmarkIcon = isSetCompleted
              ? '<svg class="w-4 h-4 text-[#3D9B6E]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2" fill="currentColor" fill-opacity="0.2"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4"/></svg>'
              : '<svg class="w-4 h-4 text-[#8A7B72]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2"/></svg>';
            const missIcon = isSetMissed
              ? '<svg class="w-4 h-4 text-[#D14040]" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-1-13h2v6h-2zm0 8h2v2h-2z"/></svg>'
              : '<svg class="w-4 h-4 text-[#8A7B72]" fill="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="9" stroke="currentColor" stroke-width="2" fill="none"/><path d="M12 7v6m0 2v2" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>';
            const hasNote = !!set.note;
            const isNoteExpanded = expandedNotes.has(i + '-' + si);
            const pencilColor = hasNote ? 'text-[#E63B57]' : 'text-[#8A7B72]';
            const noteInput = isNoteExpanded ? `
              <input type="text" value="${set.note || ''}" onchange="app.updateSet(${i}, ${si}, 'note', this.value)" placeholder="note" class="mt-1 ml-6 w-32 bg-transparent border-b border-[#DDD5CB] px-1 py-0.5 text-xs text-[#8A7B72] focus:outline-none focus:border-[#E63B57] placeholder-[#8A7B72] ${isSetCompleted ? 'opacity-50' : ''}">
              ` : '';
            return `
            <div class="py-1 border-b border-[#DDD5CB]">
              <div class="flex items-center gap-2">
                <button onclick="app.toggleSetCompleted(${i}, ${si})" class="flex-shrink-0 hover:opacity-80 transition-opacity">
                  ${setCheckmarkIcon}
                </button>
                <span class="w-6 text-[#8A7B72] text-xs ${isSetCompleted ? 'line-through' : ''}">${si + 1}</span>
                <input type="number" value="${set.weight}" onchange="app.updateSet(${i}, ${si}, 'weight', this.value)" class="w-16 bg-[#EDE6DD] border border-[#DDD5CB] rounded-lg px-2 py-1 text-center text-sm focus:outline-none focus:border-[#E63B57] text-[#3D2F2F] ${isSetCompleted ? 'opacity-50' : ''}">
                <span class="text-[#8A7B72] ${isSetCompleted ? 'line-through' : ''}">x</span>
                <input type="number" value="${set.reps}" onchange="app.updateSet(${i}, ${si}, 'reps', this.value)" class="w-14 bg-[#EDE6DD] border border-[#DDD5CB] rounded-lg px-2 py-1 text-center text-sm focus:outline-none focus:border-[#E63B57] text-[#3D2F2F] ${isSetCompleted ? 'opacity-50' : ''}">
                ${set.isPR ? (set.completed && !isSetMissed ? '<span class="text-[#E0832A] text-lg">★</span>' : '<span class="text-[#E0832A] text-lg opacity-40">★</span>') : ''}
                ...
                <button onclick="app.deleteSet(${i}, ${si})" class="text-[#D14040] hover:opacity-80 transition-opacity" title="Delete set">
        ...
        <div id="add-set-collapsed-${i}" class="mt-2 pt-2 ${ex.sets.length > 0 ? 'border-t border-[#DDD5CB]' : ''}">
          <button onclick="app.showAddSetForm(${i})" class="text-[#E63B57] text-sm">+ Add set</button>
        </div>
        <div id="add-set-expanded-${i}" class="hidden mt-2 pt-2 ${ex.sets.length > 0 ? 'border-t border-[#DDD5CB]' : ''}">
          <div class="flex items-center gap-2">
            <span class="text-[#8A7B72] text-xs w-6">${nextSetNum}</span>
            <input type="number" id="weight-${i}" class="w-16 bg-[#EDE6DD] border border-[#DDD5CB] rounded-lg px-2 py-1 text-center text-sm focus:outline-none focus:border-[#E63B57] text-[#3D2F2F]" placeholder="wt">
            <span class="text-[#8A7B72]">x</span>
            <input type="number" id="reps-${i}" class="w-14 bg-[#EDE6DD] border border-[#DDD5CB] rounded-lg px-2 py-1 text-center text-sm focus:outline-none focus:border-[#E63B57] text-[#3D2F2F]" placeholder="reps">
            <button onclick="app.saveSetInline(${i})" class="bg-[#E63B57] hover:bg-[#D03048] text-white px-3 py-1 rounded-lg text-sm transition-colors duration-200">Save</button>
            <button onclick="app.hideAddSetForm(${i})" class="text-[#8A7B72] text-sm">Cancel</button>
          </div>
          <input type="text" id="note-${i}" placeholder="note (optional)" class="mt-2 ml-6 w-40 bg-transparent border-b border-[#DDD5CB] px-1 py-0.5 text-xs text-[#8A7B72] focus:outline-none focus:border-[#E63B57] placeholder-[#8A7B72]">
        </div>
      </div>
    `;
    ...
    const checkmarkIcon = isCompleted
      ? '<svg class="w-6 h-6 text-[#3D9B6E]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2" fill="currentColor" fill-opacity="0.2"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4"/></svg>'
      : '<svg class="w-6 h-6 text-[#8A7B72]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2"/></svg>';

    return `
      <div class="bg-[#F5F0EA] rounded-2xl p-4 mb-3 shadow-[0_1px_4px_rgba(61,47,47,0.06)] border border-[#DDD5CB]">
        <div class="flex justify-between items-start mb-3">
          <div class="flex items-center gap-3">
            ...
            <div>
              <span class="font-medium ${isCompleted ? 'text-[#8A7B72] line-through' : ''}">${ex.name}</span>
              <div class="text-xs ${getTypeColor(exercise.type)}">${getTypeLabel(exercise.type)}</div>
            </div>
          </div>
          <div class="flex items-center gap-2">
            <button onclick="app.moveExerciseUp(${i})" class="text-[#8A7B72] hover:text-[#E63B57] transition-colors duration-200 ${i === 0 ? 'opacity-30 cursor-not-allowed' : ''}" ${i === 0 ? 'disabled' : ''} title="Move up">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/>
              </svg>
            </button>
            <button onclick="app.moveExerciseDown(${i})" class="text-[#8A7B72] hover:text-[#E63B57] transition-colors duration-200 ${i === exerciseCount - 1 ? 'opacity-30 cursor-not-allowed' : ''}" ${i === exerciseCount - 1 ? 'disabled' : ''} title="Move down">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
              </svg>
            </button>
            <button onclick="app.removeExercise(${i})" class="text-[#D14040] text-sm px-2 hover:text-[#B83838]">x</button>
          </div>
        </div>
        ${setsHtml}
        <div class="flex justify-between items-center mt-2">
          <button onclick="app.showExerciseNotes(${i})" class="${ex.notes ? 'text-[#E63B57]' : 'text-[#8A7B72]'} hover:text-[#D03048] transition-colors duration-200" title="${ex.notes ? 'Edit notes' : 'Add notes'}">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
            </svg>
          </button>
          <button onclick="app.showPRHistory('${ex.name.replace(/'/g, "\\'")}')" class="text-[#8A7B72] hover:text-[#E0832A] transition-colors duration-200" title="View PR history">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
