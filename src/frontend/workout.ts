...
function renderCategorySelectionGrid(): void {
  ...
  grid.innerHTML = ALL_MUSCLE_GROUPS.map(category => {
    ...
    const selectedClass = isSelected
      ? 'bg-gradient-to-r from-[#8B5CF6] to-[#06B6D4] border-[#8B5CF6] text-white shadow-[0_0_10px_rgba(139,92,246,0.3)]'
      : 'bg-white/5 border-white/10 text-[#F1F5F9]/70 hover:bg-white/10';
    return `
  ...
  list.innerHTML = exercises.map((ex, i) => {
    ...
    let setsHtml = `
      <div class="text-sm">
        ${ex.sets.length > 0 ? `
          ${ex.sets.map((set, si) => {
            ...
            const setCheckmarkIcon = isSetCompleted
              ...
              : '<svg class="w-4 h-4 text-[#64748B]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2"/></svg>';
            const missIcon = isSetMissed
              ...
              : '<svg class="w-4 h-4 text-[#64748B]" fill="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="9" stroke="currentColor" stroke-width="2" fill="none"/><path d="M12 7v6m0 2v2" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>';
            const hasNote = !!set.note;
            const isNoteExpanded = expandedNotes.has(i + '-' + si);
            const pencilColor = hasNote ? 'text-[#8B5CF6]' : 'text-[#64748B]';
            const noteInput = isNoteExpanded ? `
              <input type="text" value="${set.note || ''}" onchange="app.updateSet(${i}, ${si}, 'note', this.value)" placeholder="note" class="mt-1 ml-6 w-32 bg-transparent border-b border-white/10 px-1 py-0.5 text-xs text-[#64748B] focus:outline-none focus:border-[#8B5CF6] placeholder-[#64748B]/50 ${isSetCompleted ? 'opacity-50' : ''}">
              ` : '';
            return `
            <div class="py-1 border-b border-white/5">
              <div class="flex items-center gap-2">
                ...
                <span class="w-6 text-[#64748B] text-xs ${isSetCompleted ? 'line-through' : ''}">${si + 1}</span>
                <input type="number" value="${set.weight}" onchange="app.updateSet(${i}, ${si}, 'weight', this.value)" class="w-16 bg-white/5 border border-white/10 rounded-lg px-2 py-1 text-center text-sm text-[#F1F5F9] focus:outline-none focus:border-[#8B5CF6] ${isSetCompleted ? 'opacity-50' : ''}">
                <span class="text-[#64748B] ${isSetCompleted ? 'line-through' : ''}">x</span>
                <input type="number" value="${set.reps}" onchange="app.updateSet(${i}, ${si}, 'reps', this.value)" class="w-14 bg-white/5 border border-white/10 rounded-lg px-2 py-1 text-center text-sm text-[#F1F5F9] focus:outline-none focus:border-[#8B5CF6] ${isSetCompleted ? 'opacity-50' : ''}">
                ${set.isPR ? (set.completed && !isSetMissed ? '<span class="text-yellow-400 text-lg pr-star-glow">★</span>' : '<span class="text-yellow-400 text-lg opacity-40">★</span>') : ''}
        ...
        <div id="add-set-collapsed-${i}" class="mt-2 pt-2 ${ex.sets.length > 0 ? 'border-t border-white/5' : ''}">
          <button onclick="app.showAddSetForm(${i})" class="text-[#8B5CF6] text-sm hover:text-[#A78BFA] transition-colors">+ Add set</button>
        </div>
        <div id="add-set-expanded-${i}" class="hidden mt-2 pt-2 ${ex.sets.length > 0 ? 'border-t border-white/5' : ''}">
          <div class="flex items-center gap-2">
            <span class="text-[#64748B] text-xs w-6">${nextSetNum}</span>
            <input type="number" id="weight-${i}" class="w-16 bg-white/5 border border-white/10 rounded-lg px-2 py-1 text-center text-sm text-[#F1F5F9] focus:outline-none focus:border-[#8B5CF6]" placeholder="wt">
            <span class="text-[#64748B]">x</span>
            <input type="number" id="reps-${i}" class="w-14 bg-white/5 border border-white/10 rounded-lg px-2 py-1 text-center text-sm text-[#F1F5F9] focus:outline-none focus:border-[#8B5CF6]" placeholder="reps">
            <button onclick="app.saveSetInline(${i})" class="bg-gradient-to-r from-[#8B5CF6] to-[#06B6D4] hover:shadow-[0_0_10px_rgba(139,92,246,0.3)] text-white px-3 py-1 rounded-lg text-sm transition-all">Save</button>
            <button onclick="app.hideAddSetForm(${i})" class="text-[#64748B] text-sm hover:text-[#F1F5F9] transition-colors">Cancel</button>
          </div>
          <input type="text" id="note-${i}" placeholder="note (optional)" class="mt-2 ml-6 w-40 bg-transparent border-b border-white/10 px-1 py-0.5 text-xs text-[#64748B] focus:outline-none focus:border-[#8B5CF6] placeholder-[#64748B]/50">
        </div>
      </div>
    `;
    ...
    const checkmarkIcon = isCompleted
      ...
      : '<svg class="w-6 h-6 text-[#64748B]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2"/></svg>';
      ...
      <div class="glass-card rounded-xl p-4 mb-3">
        <div class="flex justify-between items-start mb-3">
          <div class="flex items-center gap-3">
            ...
            <div>
              <span class="font-medium ${isCompleted ? 'text-[#64748B] line-through' : ''}">${ex.name}</span>
              <div class="text-xs ${getTypeColor(exercise.type)}">${getTypeLabel(exercise.type)}</div>
            </div>
          </div>
          <div class="flex items-center gap-2">
            <button onclick="app.moveExerciseUp(${i})" class="text-[#64748B] hover:text-[#8B5CF6] transition-colors ${i === 0 ? 'opacity-30 cursor-not-allowed' : ''}" ${i === 0 ? 'disabled' : ''} title="Move up">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/>
              </svg>
            </button>
            <button onclick="app.moveExerciseDown(${i})" class="text-[#64748B] hover:text-[#8B5CF6] transition-colors ${i === exerciseCount - 1 ? 'opacity-30 cursor-not-allowed' : ''}" ${i === exerciseCount - 1 ? 'disabled' : ''} title="Move down">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
              </svg>
            </button>
            <button onclick="app.removeExercise(${i})" class="text-red-400 text-sm px-2 hover:text-red-300 transition-colors">x</button>
          </div>
        </div>
        ${setsHtml}
        <div class="flex justify-between items-center mt-2">
          <button onclick="app.showExerciseNotes(${i})" class="${ex.notes ? 'text-[#8B5CF6]' : 'text-[#64748B]'} hover:text-[#A78BFA] transition-colors" title="${ex.notes ? 'Edit notes' : 'Add notes'}">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
            </svg>
          </button>
          <button onclick="app.showPRHistory('${ex.name.replace(/'/g, "\\'")}')" class="text-[#64748B] hover:text-yellow-400 transition-colors" title="View PR history">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
