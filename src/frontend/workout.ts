...
function renderCategorySelectionGrid(): void {
  ...
  grid.innerHTML = ALL_MUSCLE_GROUPS.map(category => {
    ...
    const selectedClass = isSelected
      ? 'gradient-border-selected text-white'
      : 'bg-sunset-bg border-2 border-white/10 text-sunset-muted hover:bg-white/5';
    return `
      <button onclick="app.toggleTargetCategory('${category}')" class="p-3 rounded-xl text-sm font-medium transition-all duration-200 ${selectedClass}">
        ${category}
      </button>
    `;
  }).join('');
  ...
  list.innerHTML = exercises.map((ex, i) => {
    ...
    let setsHtml = `
      <div class="text-sm">
        ${ex.sets.length > 0 ? `
          ${ex.sets.map((set, si) => {
            ...
            const setCheckmarkIcon = isSetCompleted
              ? '<svg class="w-4 h-4 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2" fill="currentColor" fill-opacity="0.2"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4"/></svg>'
              : '<svg class="w-4 h-4 text-sunset-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2"/></svg>';
            const missIcon = isSetMissed
              ...
              : '<svg class="w-4 h-4 text-sunset-muted" fill="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="9" stroke="currentColor" stroke-width="2" fill="none"/><path d="M12 7v6m0 2v2" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>';
            const hasNote = !!set.note;
            const isNoteExpanded = expandedNotes.has(i + '-' + si);
            const pencilColor = hasNote ? 'text-sunset-rose' : 'text-sunset-muted';
            const noteInput = isNoteExpanded ? `
              <input type="text" value="${set.note || ''}" onchange="app.updateSet(${i}, ${si}, 'note', this.value)" placeholder="note" class="mt-1 ml-6 w-32 bg-transparent border-b border-white/10 px-1 py-0.5 text-xs text-sunset-muted focus:outline-none focus:border-sunset-rose placeholder-white/20 ${isSetCompleted ? 'opacity-50' : ''}">
              ` : '';
            return `
            <div class="py-1 border-b border-white/10">
              <div class="flex items-center gap-2">
                ...
                <span class="w-6 text-sunset-muted text-xs ${isSetCompleted ? 'line-through' : ''}">${si + 1}</span>
                <input type="number" value="${set.weight}" onchange="app.updateSet(${i}, ${si}, 'weight', this.value)" class="w-16 bg-sunset-bg border border-white/10 rounded-lg px-2 py-1 text-center text-sm text-sunset-text focus:outline-none focus:border-sunset-rose transition-colors duration-200 ${isSetCompleted ? 'opacity-50' : ''}">
                <span class="text-sunset-muted ${isSetCompleted ? 'line-through' : ''}">x</span>
                <input type="number" value="${set.reps}" onchange="app.updateSet(${i}, ${si}, 'reps', this.value)" class="w-14 bg-sunset-bg border border-white/10 rounded-lg px-2 py-1 text-center text-sm text-sunset-text focus:outline-none focus:border-sunset-rose transition-colors duration-200 ${isSetCompleted ? 'opacity-50' : ''}">
                ${set.isPR ? (set.completed && !isSetMissed ? '<span class="text-orange-400 text-lg pr-star-glow">★</span>' : '<span class="text-orange-400 text-lg opacity-40">★</span>') : ''}
                <button onclick="app.toggleSetMissed(${i}, ${si})" class="flex-shrink-0 hover:opacity-80 transition-all duration-200" title="${isSetMissed ? 'Mark as not missed' : 'Mark as missed'}">
                  ${missIcon}
                </button>
                <button onclick="app.toggleNoteField(${i}, ${si})" class="${pencilColor} text-sm hover:opacity-80 transition-all duration-200" title="${hasNote ? 'Edit note' : 'Add note'}">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path>
                  </svg>
                </button>
                <button onclick="app.deleteSet(${i}, ${si})" class="text-red-400 hover:opacity-80 transition-all duration-200" title="Delete set">
        ...
        <div id="add-set-collapsed-${i}" class="mt-2 pt-2 ${ex.sets.length > 0 ? 'border-t border-white/10' : ''}">
          <button onclick="app.showAddSetForm(${i})" class="text-sunset-rose text-sm transition-colors duration-200 hover:text-sunset-orange">+ Add set</button>
        </div>
        <div id="add-set-expanded-${i}" class="hidden mt-2 pt-2 ${ex.sets.length > 0 ? 'border-t border-white/10' : ''}">
          <div class="flex items-center gap-2">
            <span class="text-sunset-muted text-xs w-6">${nextSetNum}</span>
            <input type="number" id="weight-${i}" class="w-16 bg-sunset-bg border border-white/10 rounded-lg px-2 py-1 text-center text-sm text-sunset-text focus:outline-none focus:border-sunset-rose transition-colors duration-200" placeholder="wt">
            <span class="text-sunset-muted">x</span>
            <input type="number" id="reps-${i}" class="w-14 bg-sunset-bg border border-white/10 rounded-lg px-2 py-1 text-center text-sm text-sunset-text focus:outline-none focus:border-sunset-rose transition-colors duration-200" placeholder="reps">
            <button onclick="app.saveSetInline(${i})" class="sunset-gradient text-white px-3 py-1 rounded-lg text-sm transition-all duration-200 hover:brightness-110">Save</button>
            <button onclick="app.hideAddSetForm(${i})" class="text-sunset-muted text-sm transition-colors duration-200 hover:text-sunset-text">Cancel</button>
          </div>
          <input type="text" id="note-${i}" placeholder="note (optional)" class="mt-2 ml-6 w-40 bg-transparent border-b border-white/10 px-1 py-0.5 text-xs text-sunset-muted focus:outline-none focus:border-sunset-rose placeholder-white/20 transition-colors duration-200">
        </div>
      </div>
    `;
    ...
    const checkmarkIcon = isCompleted
      ? '<svg class="w-6 h-6 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2" fill="currentColor" fill-opacity="0.2"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4"/></svg>'
      : '<svg class="w-6 h-6 text-sunset-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2"/></svg>';

    return `
      <div class="bg-sunset-card border border-white/10 rounded-xl p-4 mb-3 exercise-card-gradient">
        <div class="flex justify-between items-start mb-3">
          <div class="flex items-center gap-3">
            <button onclick="app.toggleExerciseCompleted(${i})" class="flex-shrink-0 hover:opacity-80 transition-all duration-200">
              ${checkmarkIcon}
            </button>
            <div>
              <span class="font-medium ${isCompleted ? 'text-sunset-muted line-through' : ''}">${ex.name}</span>
              <div class="text-xs ${getTypeColor(exercise.type)}">${getTypeLabel(exercise.type)}</div>
            </div>
          </div>
          <div class="flex items-center gap-2">
            <button onclick="app.moveExerciseUp(${i})" class="text-sunset-muted hover:text-sunset-rose transition-colors duration-200 ${i === 0 ? 'opacity-30 cursor-not-allowed' : ''}" ${i === 0 ? 'disabled' : ''} title="Move up">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/>
              </svg>
            </button>
            <button onclick="app.moveExerciseDown(${i})" class="text-sunset-muted hover:text-sunset-rose transition-colors duration-200 ${i === exerciseCount - 1 ? 'opacity-30 cursor-not-allowed' : ''}" ${i === exerciseCount - 1 ? 'disabled' : ''} title="Move down">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
              </svg>
            </button>
            <button onclick="app.removeExercise(${i})" class="text-red-400 text-sm px-2 hover:text-red-300 transition-colors duration-200">x</button>
          </div>
        </div>
        ${setsHtml}
        <div class="flex justify-between items-center mt-2">
          <button onclick="app.showExerciseNotes(${i})" class="${ex.notes ? 'text-sunset-rose' : 'text-sunset-muted'} hover:text-sunset-orange transition-colors duration-200" title="${ex.notes ? 'Edit notes' : 'Add notes'}">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
            </svg>
          </button>
          <button onclick="app.showPRHistory('${ex.name.replace(/'/g, "\\'")}')" class="text-sunset-muted hover:text-orange-400 transition-colors duration-200" title="View PR history">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
