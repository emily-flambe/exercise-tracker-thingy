...
function renderCategorySelectionGrid(): void {
  ...
  grid.innerHTML = ALL_MUSCLE_GROUPS.map(category => {
    ...
    return `
      <button onclick="app.toggleTargetCategory('${category}')" class="p-3 rounded-sm border-2 text-sm font-bold transition-colors uppercase tracking-wider ${selectedClass}">
        ${category}
      </button>
    `;
  }).join('');
  ...
  list.innerHTML = exercises.map((ex, i) => {
    ...
    let setsHtml = `
      <div class="text-sm">
        ${ex.sets.length > 0 ? `
          ${ex.sets.map((set, si) => {
            ...
            const setCheckmarkIcon = isSetCompleted
              ? '<svg class="w-4 h-4 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2" fill="currentColor" fill-opacity="0.15"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4"/></svg>'
              : '<svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2"/></svg>';
            const missIcon = isSetMissed
              ? '<svg class="w-4 h-4 text-swiss-red" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-1-13h2v6h-2zm0 8h2v2h-2z"/></svg>'
              : '<svg class="w-4 h-4 text-gray-400" fill="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="9" stroke="currentColor" stroke-width="2" fill="none"/><path d="M12 7v6m0 2v2" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>';
            const hasNote = !!set.note;
            const isNoteExpanded = expandedNotes.has(i + '-' + si);
            const pencilColor = hasNote ? 'text-swiss-red' : 'text-gray-400';
            const noteInput = isNoteExpanded ? `
              <input type="text" value="${set.note || ''}" onchange="app.updateSet(${i}, ${si}, 'note', this.value)" placeholder="note" class="mt-1 ml-6 w-32 bg-transparent border-b-2 border-gray-300 px-1 py-0.5 text-xs text-gray-500 focus:outline-none focus:border-swiss-red placeholder-gray-400 ${isSetCompleted ? 'opacity-50' : ''}">
              ` : '';
            return `
            <div class="py-1 border-b border-gray-300">
              <div class="flex items-center gap-2">
                ...
                <input type="number" value="${set.weight}" onchange="app.updateSet(${i}, ${si}, 'weight', this.value)" class="w-16 bg-white border-2 border-black rounded-sm px-2 py-1 text-center text-sm font-mono focus:outline-none focus:border-swiss-red ${isSetCompleted ? 'opacity-50' : ''}">
                <span class="text-gray-500 ${isSetCompleted ? 'line-through' : ''}">x</span>
                <input type="number" value="${set.reps}" onchange="app.updateSet(${i}, ${si}, 'reps', this.value)" class="w-14 bg-white border-2 border-black rounded-sm px-2 py-1 text-center text-sm font-mono focus:outline-none focus:border-swiss-red ${isSetCompleted ? 'opacity-50' : ''}">
                ${set.isPR ? (set.completed && !isSetMissed ? '<span class="text-swiss-red text-lg">★</span>' : '<span class="text-swiss-red text-lg opacity-40">★</span>') : ''}
                ...
                <button onclick="app.deleteSet(${i}, ${si})" class="text-swiss-red hover:opacity-80 transition-opacity" title="Delete set">
        ...
        <div id="add-set-collapsed-${i}" class="mt-2 pt-2 ${ex.sets.length > 0 ? 'border-t border-gray-300' : ''}">
          <button onclick="app.showAddSetForm(${i})" class="text-swiss-red text-sm font-bold uppercase tracking-wider">+ Add set</button>
        </div>
        <div id="add-set-expanded-${i}" class="hidden mt-2 pt-2 ${ex.sets.length > 0 ? 'border-t border-gray-300' : ''}">
          <div class="flex items-center gap-2">
            <span class="text-gray-500 text-xs w-6 font-mono">${nextSetNum}</span>
            <input type="number" id="weight-${i}" class="w-16 bg-white border-2 border-black rounded-sm px-2 py-1 text-center text-sm font-mono focus:outline-none focus:border-swiss-red" placeholder="wt">
            <span class="text-gray-500">x</span>
            <input type="number" id="reps-${i}" class="w-14 bg-white border-2 border-black rounded-sm px-2 py-1 text-center text-sm font-mono focus:outline-none focus:border-swiss-red" placeholder="reps">
            <button onclick="app.saveSetInline(${i})" class="bg-black hover:bg-gray-800 text-white px-3 py-1 rounded-sm text-sm font-bold uppercase">Save</button>
            <button onclick="app.hideAddSetForm(${i})" class="text-gray-500 text-sm">Cancel</button>
          </div>
          <input type="text" id="note-${i}" placeholder="note (optional)" class="mt-2 ml-6 w-40 bg-transparent border-b-2 border-gray-300 px-1 py-0.5 text-xs text-gray-500 focus:outline-none focus:border-swiss-red placeholder-gray-400">
        </div>
      </div>
    `;
    ...
    const checkmarkIcon = isCompleted
      ? '<svg class="w-6 h-6 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2" fill="currentColor" fill-opacity="0.15"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4"/></svg>'
    ...
    return `
      <div class="bg-white border-2 border-black rounded-sm p-4 mb-3">
        <div class="flex justify-between items-start mb-3">
          <div class="flex items-center gap-3">
            ...
            <div>
              <span class="font-bold ${isCompleted ? 'text-gray-400 line-through' : ''}">${ex.name}</span>
              <div class="text-xs ${getTypeColor(exercise.type)}">${getTypeLabel(exercise.type)}</div>
            </div>
          </div>
          <div class="flex items-center gap-2">
            <button onclick="app.moveExerciseUp(${i})" class="text-gray-400 hover:text-black transition-colors ${i === 0 ? 'opacity-30 cursor-not-allowed' : ''}" ${i === 0 ? 'disabled' : ''} title="Move up">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"/>
              </svg>
            </button>
            <button onclick="app.moveExerciseDown(${i})" class="text-gray-400 hover:text-black transition-colors ${i === exerciseCount - 1 ? 'opacity-30 cursor-not-allowed' : ''}" ${i === exerciseCount - 1 ? 'disabled' : ''} title="Move down">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
              </svg>
            </button>
            <button onclick="app.removeExercise(${i})" class="text-swiss-red text-sm px-2 hover:opacity-70">x</button>
          </div>
        </div>
        ${setsHtml}
        <div class="flex justify-between items-center mt-2">
          <button onclick="app.showExerciseNotes(${i})" class="${ex.notes ? 'text-swiss-red' : 'text-gray-400'} hover:text-swiss-red transition-colors" title="${ex.notes ? 'Edit notes' : 'Add notes'}">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
            </svg>
          </button>
          <button onclick="app.showPRHistory('${ex.name.replace(/'/g, "\\'")}')" class="text-gray-400 hover:text-swiss-red transition-colors" title="View PR history">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function switchTabDirect(tabName: string): void {
  ...
  document.querySelectorAll('nav button').forEach(btn => {
    btn.classList.remove('text-swiss-red');
    btn.classList.add('text-gray-500');
  });
  $('nav-' + tabName).classList.remove('text-gray-500');
  $('nav-' + tabName).classList.add('text-swiss-red');
}
